...
public class U2Practica3 {
    static void printMenu() {
        ...
        System.out.println("\t6) Libro de historia en el tope");
        System.out.println("\t7) Aumentar precio de todos los libros");
        System.out.println("\t8) Vaciar la pila de Novelas");
        System.out.println("\t9) Precio total de libros");
    ...
    static void getHistoryBookOnStackTop(PilaLibros novela) {
        Libro actual = novela.peek();
        
        if (actual == null) {
            System.err.println("NO HAY LIBROS DE HISTORIA");
            return;
        }
        
        System.out.printf("Libro {\n\tTitulo: %s\n\tAutor: %s\n}%n", actual.getTitulo(), actual.getAutor());
        
    }
    
    static void raiseAllBooksPrices(PilaLibros historia, PilaLibros novela, int raise) {
        float floatRaise = 1.0f + (float) raise / 100;
        
        historia.raiseBooksPrices(floatRaise);
        novela.raiseBooksPrices(floatRaise);
    }
    
    static void clearNovelsStackContent(PilaLibros novela) {
        String status = novela.clearStack();
        
        if (Objects.isNull(status)) {
            System.err.println("NO HAY LIBROS DE NOVELA PARA ELIMINAR");
            return;
        }
    ...
    static void getTotalPrice(PilaLibros novela, PilaLibros historia) {
        System.out.println("Precio total: " + (novela.getTotalPrice() + historia.getTotalPrice()));
    }
        ...
        do {
            try {
                ...
                switch (option) {
                    ...
                    case 6 -> getHistoryBookOnStackTop(historia);
                    case 7 -> {
                        System.out.print("Porcentaje a aumentar: ");
                        int raise = Integer.parseInt(br.readLine());
                        
                        raiseAllBooksPrices(historia, novela, raise);
                    }
                    case 8 -> clearNovelsStackContent(novela);
                    case 9 -> getTotalPrice(novela, historia);
